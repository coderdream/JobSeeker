...
public class Job51 {
    ...
    private int confirmedDeliveredCount = 0;
    ...
    // 当前页从JSON拦截到的jobId列表
    private final List<Long> currentPageJobIds = new ArrayList<>();
    ...
    public int execute() {
        ...
        try {
            ...
            String message = String.format("51job投递完成，共投递%d个简历，用时%s",
                confirmedDeliveredCount, formatDuration(duration));
        ...
        return confirmedDeliveredCount;
    }
    ...
    private void deliverByKeyword(String keyword, String searchUrl) {
        try {
            ...
            try {
                page.waitForSelector(Job51PageModel.JOB_CARD_SELECTOR,
                        new Page.WaitForSelectorOptions().setTimeout(10_000));
            } catch (Exception e) {
                log.warn("[51job] 等待职位列表超时: {}", e.getMessage());
                sendProgress("未找到 51job 职位列表，可能页面结构变化或搜索结果为空", null, null);
                return;
            }
            ...
            for (int pageNum = 1; pageNum <= DEFAULT_MAX_PAGE; pageNum++) {
                ...
                if (pageNum < DEFAULT_MAX_PAGE && !goToNextPage()) {
                    break;
                }
            }
    ...
    private void deliverCurrentPage() {
        try {
            ...
            Locator cards = page.locator(Job51PageModel.JOB_CARD_SELECTOR);
            int jobCount = cards.count();
            if (jobCount == 0) {
                log.warn("[51job] 当前页未找到职位卡片");
                sendProgress("当前页未找到职位卡片，可能页面结构变化", null, null);
                reachedDailyLimit = true;
                return;
            }

            log.info("[51job] 当前页检测到 {} 个职位卡片", jobCount);
            ...
            for (int i = 0; i < jobCount; i++) {
                ...
                try {
                    ...
                    Locator applyButtons = card.locator(Job51PageModel.PER_JOB_APPLY_BUTTON_SELECTOR);
                    if (applyButtons.count() == 0) {
                        log.debug("[51job] 第 {} 个职位未找到投递按钮", i + 1);
                        continue;
                    }

                    String title = safeText(card.locator(".joblist-item-jobname"));
                    String company = safeText(card.locator(".comp"));
                    String jobInfo = (company.isBlank() ? "未知公司" : company) + " | " + (title.isBlank() ? "未知职位" : title);

                    try {
                        applyButtons.first().scrollIntoViewIfNeeded();
                    } catch (Exception ignored) {
                    }
                    applyButtons.first().click(new Locator.ClickOptions().setTimeout(5_000));
                    PlaywrightUtil.sleep(2);

                    OptionalInt successCount = handleDeliverySuccessDialog();
                    if (successCount.isPresent() && successCount.getAsInt() > 0) {
                        confirmedDeliveredCount += successCount.getAsInt();
                        resultList.add(jobInfo);
                        markDeliveredForCurrentCard(i);
                    } else {
                    ...
                    log.warn("[51job] 投递第 {} 个职位失败: {}", i + 1, e.getMessage());
                }
            }
    ...
    private void markDeliveredForCurrentCard(int cardIndex) {
        try {
            Long jobId = null;
            synchronized (currentPageJobIds) {
                if (cardIndex >= 0 && cardIndex < currentPageJobIds.size()) {
                    jobId = currentPageJobIds.get(cardIndex);
                }
            }
            if (jobId == null) {
                log.debug("[51job] 当前职位没有缓存的 jobId，跳过状态标记");
                return;
            }
            job51Service.markDeliveredBatch(List.of(jobId));
            log.info("[51job] 标记已投递职位 jobId={}", jobId);
        } catch (Exception e) {
     ...
     * 处理投递成功弹窗
     */
    private OptionalInt handleDeliverySuccessDialog() {
        try {
            PlaywrightUtil.sleep(2);
            ...
            Locator elDialogBody = page.locator(Job51PageModel.SUCCESS_DIALOG_BODY_SELECTOR);
            if (elDialogBody.count() > 0) {
                String dialogText = elDialogBody.first().innerText();
                OptionalInt parsedSuccessCount = Job51PageModel.parseSuccessfulDeliveryCount(dialogText);
                if (parsedSuccessCount.isPresent()) {
    ...
    private boolean goToNextPage() {
        try {
            closeAnyModalOverlays();
            Locator disabledNext = page.locator(Job51PageModel.DISABLED_NEXT_PAGE_BUTTON_SELECTOR);
            if (disabledNext.count() > 0 && disabledNext.first().isVisible()) {
                log.info("[51job] 下一页按钮不可用，结束翻页");
                return false;
            }

            Locator nextButton = page.locator(Job51PageModel.NEXT_PAGE_BUTTON_SELECTOR);
            if (nextButton.count() == 0) {
                log.info("[51job] 未找到下一页按钮，结束翻页");
                return false;
            }

            nextButton.first().scrollIntoViewIfNeeded();
            nextButton.first().click(new Locator.ClickOptions().setTimeout(5_000));
            PlaywrightUtil.sleep(2);
            ...
            try {
                ...
                log.warn("[51job] 翻页后等待职位列表超时: {}", e.getMessage());
                return false;
            }
            return true;
        } catch (Exception e) {
            log.warn("[51job] 点击下一页失败: {}", e.getMessage());
            return false;
        }
    }
    ...
    private void closeAnyModalOverlays() {
        try {
            ...
            for (int t = 0; t < 3; t++) {
                ...
                Locator headerClose = page.locator(Job51PageModel.CLOSE_BUTTON_SELECTOR);
                ...
                if (!closedThisRound) {
                    try {
                        page.evaluate("document.querySelectorAll('button.el-dialog__headerbtn, button[aria-label=\\'Close\\'], .van-popup__close-icon, .van-icon-cross, [class*=close]').forEach(b=>b.click())");
                    } catch (Exception ignored) {}
                }
                Locator popupClose = page.locator(".van-popup__close-icon, .van-icon-cross, [class*='close']");
            ...
            }
            {
                ...
                try { page.waitForSelector(Job51PageModel.BLOCKING_OVERLAY_SELECTOR, new Page.WaitForSelectorOptions().setState(WaitForSelectorState.DETACHED).setTimeout(3000)); } catch (Exception ignored) {}
            }
        } catch (Exception e) {
