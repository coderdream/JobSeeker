...
public class LiepinService {
    ...
    public void ensureTableExists() {
        if (!Boolean.getBoolean("jobs.allowRuntimeDdl")) {
            log.debug("hub_liepin_data schema is managed by Flyway");
            return;
        }
        String createSql = "CREATE TABLE IF NOT EXISTS hub_liepin_data (" +
        ...
        try (Connection conn = dataSource.getConnection(); Statement stmt = conn.createStatement()) {
            ...
            try {
                stmt.execute("ALTER TABLE hub_liepin_data ADD COLUMN delivered INTEGER DEFAULT 0");
            } catch (Exception ignored) {}
            // 兼容旧库：尝试移除无数据列（SQLite 3.35+ 支持；不支持则忽略错误）
            try { stmt.execute("ALTER TABLE hub_liepin_data DROP COLUMN job_function"); } catch (Exception ignored) {}
            try { stmt.execute("ALTER TABLE hub_liepin_data DROP COLUMN job_city"); } catch (Exception ignored) {}
            try { stmt.execute("ALTER TABLE hub_liepin_data DROP COLUMN comp_full_name"); } catch (Exception ignored) {}
            try { stmt.execute("ALTER TABLE hub_liepin_data DROP COLUMN comp_kind"); } catch (Exception ignored) {}
            log.info("hub_liepin_data schema ensured");
        } catch (Exception e) {
            log.warn("创建 hub_liepin_data 表失败: {}", e.getMessage());
        }
    }
     ...
     */
    public void insertSnapshotsIfNotExistsBatch(List<LiepinEntity> entities) {
        ...
        List<LiepinEntity> existing = liepinMapper.selectList(
        ...
        // 过滤出需要插入的记录
        List<LiepinEntity> toInsert = new ArrayList<>();
        LocalDateTime now = LocalDateTime.now();
        for (LiepinEntity e : entities) {
        ...
        // 使用JDBC批量插入以提升性能
        String sql = "INSERT INTO hub_liepin_data (" +
    ...
    public LiepinConfigEntity getFirstConfig() {
        return getFirstConfig(currentUserService.requireUserId());
    }

    public LiepinConfigEntity getFirstConfig(Long userId) {
    ...
    public LiepinConfigEntity updateConfig(LiepinConfigEntity config) {
        ...
        LiepinConfigEntity existing = getFirstConfig(userId);
        if (existing == null) {
            config.setId(null);
        ...
        config.setUserId(userId);
        config.setUpdatedAt(LocalDateTime.now());
        liepinConfigMapper.update(config, new UpdateWrapper<LiepinConfigEntity>()
                .eq("id", existing.getId())
                .eq("user_id", userId));
        return liepinConfigMapper.selectById(existing.getId());
    }
    ...
    public LiepinConfigEntity saveOrUpdateFirstSelective(LiepinConfigEntity config) {
        ...
        if (config.getCompTag() != null) {
            existing.setCompTag(config.getCompTag());
        }
        if (config.getPubTime() != null) {
            existing.setPubTime(config.getPubTime());
        }
        if (config.getWorkYearCode() != null) {
            existing.setWorkYearCode(config.getWorkYearCode());
        }
        if (config.getEduLevel() != null) {
            existing.setEduLevel(config.getEduLevel());
        }
        if (config.getIndustry() != null) {
            existing.setIndustry(config.getIndustry());
        }
        if (config.getJobKind() != null) {
            existing.setJobKind(config.getJobKind());
        }
        if (config.getCompScale() != null) {
            existing.setCompScale(config.getCompScale());
        }
        if (config.getCompStage() != null) {
            existing.setCompStage(config.getCompStage());
        }
        if (config.getCompKind() != null) {
            existing.setCompKind(config.getCompKind());
        }
        existing.setUserId(userId);
        existing.setUpdatedAt(LocalDateTime.now());
        liepinConfigMapper.update(existing, new UpdateWrapper<LiepinConfigEntity>()
                .eq("id", existing.getId())
                .eq("user_id", userId));
        return liepinConfigMapper.selectById(existing.getId());
    }
    ...
    public List<LiepinOptionEntity> getOptionsByType(String type) {
        List<ReferenceDataService.OptionItem> items = "city".equals(type)
                ? referenceDataService.listCityOptionsForPlatform("liepin")
                : referenceDataService.listPlatformOptionItems("liepin", type);
        if (!items.isEmpty()) {
            return items.stream().map(this::toLiepinOption).collect(Collectors.toList());
        }
    ...
    public LiepinOptionEntity getOptionByTypeAndCode(String type, String code) {
        String name = referenceDataService.nameByCode("liepin", type, code);
    ...
    public String getCodeByTypeAndName(String type, String name) {
        String code = referenceDataService.codeByName("liepin", type, name);
    ...
    private LiepinOptionEntity toLiepinOption(ReferenceDataService.OptionItem item) {
        LiepinOptionEntity entity = new LiepinOptionEntity();
    ...
    public static SalaryInfo parseSalary(String salaryText) {
        ...
        Integer months = null;
        java.util.regex.Matcher mMonths = java.util.regex.Pattern.compile("([0-9]{1,2})[\\u00B7\\u00D7xX*]?(?:\\u85AA|$)").matcher(s);
        if (mMonths.find()) {
            try { months = Integer.parseInt(mMonths.group(1)); } catch (Exception ignored) {}
        }

        String cleaned = s.replaceAll("[路\u00B7\u00D7xX*].*$", "");
