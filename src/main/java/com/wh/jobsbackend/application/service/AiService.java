...
public class AiService {
    ...
    public AiEntity getAiConfig() {
        LambdaQueryWrapper<AiEntity> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(AiEntity::getUserId, currentUserService.requireUserId())
                .orderByDesc(AiEntity::getUpdatedAt)
                .last("LIMIT 1");
        var list = aiMapper.selectList(wrapper);
    ...
    public java.util.List<AiEntity> getAllAiConfigs() {
        LambdaQueryWrapper<AiEntity> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(AiEntity::getUserId, currentUserService.requireUserId());
        return aiMapper.selectList(wrapper);
    }
    ...
    public AiEntity saveOrUpdateAiConfig(String introduce, String prompt) {
        ...
        if (aiEntity == null) {
            ...
            aiEntity.setUserId(currentUserService.requireUserId());
