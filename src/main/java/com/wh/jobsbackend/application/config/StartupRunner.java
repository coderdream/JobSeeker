...
@Slf4j
@Component
@ConditionalOnProperty(prefix = "app.startup", name = "enabled", havingValue = "true", matchIfMissing = true)
public class StartupRunner implements ApplicationRunner {

    @Value("${server.port:8888}")
    private int backendPort;

    @Value("${app.startup.playwright.enabled:false}")
    private boolean initializePlaywright;
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        if (initializePlaywright) {
            ...
            log.info("跳过启动期 Playwright 初始化；将在平台登录/任务触发时按需启动");
        }
    }
    ...
    private boolean hasStaticResources() {
        try {
            ...
            if (new ClassPathResource("dist/index.html").exists() || new ClassPathResource("static/index.html").exists()) {
                return true;
            }
        } catch (Exception e) {
