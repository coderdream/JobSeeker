...
export default function AnalysisContent({ showHeader = false }: { showHeader?: boolean }) {
  ...
  const loadList = useCallback(async (toPage: number, toSize: number) => {
    try {
      ...
      const res = await authedFetch(`/api/zhilian/list?${params.toString()}`)
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      if (maxK) params.set("maxK", String(Number(maxK)))
      if (keyword) params.set("keyword", keyword)
      const res = await authedFetch(`/api/zhilian/stats?${params.toString()}`)
  ...
  const refreshComputedSalaryBuckets = useCallback(async () => {
    try {
      const baseParams = new URLSearchParams()
      if (statuses.length) baseParams.set("statuses", statuses.join(","))
      if (location) baseParams.set("location", location)
  ...
  useEffect(() => {
    ...
    if (apiBuckets.length === 0 || sum === 0) {
      queueMicrotask(() => void refreshComputedSalaryBuckets())
    } else {
      queueMicrotask(() => setComputedSalaryBuckets([]))
    }
  }, [refreshComputedSalaryBuckets, stats])
      ...
      <Card>
        <CardHeader className="space-y-0">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2 rounded-md border border-border bg-muted/40 p-2">
              {statusOptions.map((s) => (
                <button
                  key={s}
                  onClick={() => setStatuses((prev) => (prev.includes(s) ? prev.filter((x) => x !== s) : [...prev, s]))}
                  className={`rounded-md border px-3 py-1.5 text-xs transition-colors ${statuses.includes(s) ? "border-primary/40 bg-primary/10 text-primary" : "border-border bg-background text-muted-foreground hover:bg-accent hover:text-foreground"}`}
                >
                  {s}
                </button>
              ))}
              <button className="rounded-md border border-border bg-background px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:bg-accent hover:text-foreground" onClick={() => setStatuses([])}>重置</button>
            </div>
          </div>
        </CardHeader>
        <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            {stats ? (
              <ChartCanvas
                type="line"
                labels={(computedSalaryBuckets.length ? computedSalaryBuckets : stats.charts.salaryBuckets.length ? stats.charts.salaryBuckets : fallbackSalaryBuckets).map((x) => x.bucket)}
                data={(computedSalaryBuckets.length ? computedSalaryBuckets : stats.charts.salaryBuckets.length ? stats.charts.salaryBuckets : fallbackSalaryBuckets).map((x) => x.value)}
                color="#ef4444"
              />
            ) : (
              <div className="text-muted-foreground">加载中...</div>
