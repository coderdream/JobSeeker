...
import ChartCanvas from "@/components/workbench/ChartCanvas"
...
export default function AnalysisContent({ showHeader = false }:{ showHeader?: boolean }) {
  ...
  const loadList = useCallback(async (toPage: number, toSize: number)=>{
    ...
    try{ setLoadingList(true); const res = await authedFetch(`/api/51job/list?${params.toString()}`); const data:PagedResult51 = await res.json(); const nextPage=data.page||toPage; const nextSize=data.size||toSize; setItems(data.items||[]); setTotal(data.total||0); setPage(nextPage); setSize(nextSize); setInputPage(nextPage); setInputSize(nextSize) }catch(e){ console.error("fetch list failed",e) } finally { setLoadingList(false) }
  }, [authedFetch, degree, experience, keyword, location, maxK, minK, statuses])
  ...
  const loadStats = useCallback(async ()=>{
    ...
    if (maxK) params.set("maxK", String(Number(maxK)))
    if (keyword) params.set("keyword", keyword)
    try{ setLoadingStats(true); const res = await authedFetch(`/api/51job/stats?${params.toString()}`); const data:StatsResponse = await res.json(); setStats(data) }catch(e){ console.error("fetch stats failed",e) } finally { setLoadingStats(false) }
  }, [authedFetch, degree, experience, keyword, location, maxK, minK, statuses])

  useEffect(()=>{ queueMicrotask(() => void loadStats()) },[loadStats])
  useEffect(()=>{ queueMicrotask(() => void loadList(1,size)) },[loadList, size])

  const onReload = async ()=>{
    try{ setReloading(true); const res=await authedFetch('/api/51job/reload'); const data=await res.json(); console.log("reload",data); await loadList(1,size); await loadStats() }catch(e){ console.error("reload failed",e) } finally { setReloading(false) }
  }
  ...
  return (
    <div className="min-w-0 max-w-full space-y-5 overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {kpiCards.map((c,idx)=> (
          <Card key={idx}><CardHeader className="p-4"><CardTitle className="text-sm">{c.title}</CardTitle><CardDescription className="text-lg font-semibold">{c.value}</CardDescription></CardHeader></Card>
        ))}
      </div>
        ...
        <CardHeader className="space-y-0">
          <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex max-w-full flex-wrap gap-2 rounded-md border border-border bg-muted/40 p-2">
              {statusOptions.map(s=> (
                <label key={s} className={`group inline-flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm transition-colors ${statuses.includes(s)?"border-primary/40 bg-primary/10 text-primary":"border-border bg-background text-muted-foreground hover:bg-accent hover:text-foreground"}`}>
