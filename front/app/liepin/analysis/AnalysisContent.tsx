...
export default function AnalysisContent({ showHeader = false }: { showHeader?: boolean }) {
  ...
  const loadList = useCallback(async (toPage: number, toSize: number) => {
    ...
    try {
      setLoadingList(true)
      const res = await authedFetch(`/api/liepin/list?${params.toString()}`)
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      const res = await authedFetch(`/api/liepin/stats?${params.toString()}`)
  ...
  return (
    <div className="min-w-0 max-w-full space-y-6 overflow-hidden">
        ...
        <PageHeader
          ...
          actions={
            <div className="flex flex-wrap items-center gap-2">
              <Button onClick={() => { loadList(1, size); loadStats(); }} size="sm" className="px-3">
                <BiRefresh className="mr-1" /> 刷新
              </Button>
              <Button onClick={exportCSV} disabled={exporting} size="sm" variant="outline" className="px-3">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <Button onClick={() => { loadList(1, size); loadStats(); }} className="px-3">
              <BiRefresh className="mr-1" /> 应用筛选
            </Button>
            <Button onClick={exportCSV} disabled={exporting} variant="outline" className="px-3">
              <BiDownload className="mr-1" /> 导出CSV
            </Button>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="max-h-[560px] max-w-full overflow-x-auto overflow-y-auto">
            <table className="min-w-full text-sm table-fixed">
              ...
              <tbody>
                {(items || []).map((it) => (
                  <tr key={it.jobId} className="border-t border-border">
          ...
          {typeof detailJob !== "undefined" && detailJob && (
            <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setDetailJob(null)}>
              <div className="w-[800px] max-w-[90vw] rounded-lg border border-border bg-background p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold">岗位详情</h3>
                  <Button variant="outline" className="px-3" onClick={() => setDetailJob(null)}>关闭</Button>
                </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-2">
              <Button onClick={() => loadList(page - 1 > 0 ? page - 1 : 1, size)} disabled={loadingList || page <= 1} variant="outline" className="px-3">
                上一页
              </Button>
              <Button onClick={() => loadList(page + 1, size)} disabled={loadingList || items.length < size} variant="outline" className="px-3">
                下一页
              </Button>
            </div>
          </div>
